...
export default function GeoStatsExplorer({
  ...
  function drillInto(child: ChildUnit) {
    setFocusStack((prev) => [
      ...prev,
      {
            ...
            {data.top_users.map((u, i) => {
              ...
              return (
                <li key={u.user_id} className="px-5 py-3 flex items-center gap-3">
                  <RankBadge rank={i + 1} />
                  <div className="flex items-center gap-2.5 flex-1 min-w-0">
                    <div className="w-7 h-7 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-xs font-bold shrink-0 text-zinc-400">
                      {name[0].toUpperCase()}
                    </div>
