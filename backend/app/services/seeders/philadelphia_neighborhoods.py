...
from app.services.geo import SIMPLIFIED_PHILADELPHIA_NEIGHBORHOODS_FILE
...
class PhiladelphiaNeighborhoodSeeder(Seeder):
    """
    Seeds Philadelphia neighborhood boundaries into geo_units for the toggleable
    ...
    async def run(self, db: AsyncSession, params: dict) -> SeedResult:
        if not SIMPLIFIED_PHILADELPHIA_NEIGHBORHOODS_FILE.exists():
            raise FileNotFoundError(
                f"Simplified file not found: {SIMPLIFIED_PHILADELPHIA_NEIGHBORHOODS_FILE}. "
                "Run POST /admin/simplify-philadelphia-neighborhoods first."
            )

        with open(SIMPLIFIED_PHILADELPHIA_NEIGHBORHOODS_FILE, encoding="utf-8") as f:
                ...
                try:
                    await db.execute(
                        text("""
                            ...
                            VALUES (
                                :unit_id, 'philadelphia_neighborhood',
    ...
    async def _compute_adjacency(self, db: AsyncSession) -> None:
        await db.execute(
            text("""
                ...
                WHERE geo_unit_id IN (SELECT id FROM geo_units WHERE unit_type = 'philadelphia_neighborhood')
            """)
        )
        await db.execute(
            text("""
