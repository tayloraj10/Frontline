...
from app.services.geo import SIMPLIFIED_LA_NEIGHBORHOODS_FILE
...
class LaNeighborhoodSeeder(Seeder):
    """
    Seeds LA (City of Los Angeles) neighborhood boundaries into geo_units for the
    toggleable "mosaic" overlay layer, same pattern as PhiladelphiaNeighborhoodSeeder.
    ...
    async def run(self, db: AsyncSession, params: dict) -> SeedResult:
        if not SIMPLIFIED_LA_NEIGHBORHOODS_FILE.exists():
            raise FileNotFoundError(
                f"Simplified file not found: {SIMPLIFIED_LA_NEIGHBORHOODS_FILE}. "
                "Run POST /admin/simplify-la-neighborhoods first."
            )

        with open(SIMPLIFIED_LA_NEIGHBORHOODS_FILE, encoding="utf-8") as f:
    ...
    async def _compute_adjacency(self, db: AsyncSession) -> None:
        await db.execute(
            text("""
                ...
                WHERE geo_unit_id IN (SELECT id FROM geo_units WHERE unit_type = 'la_neighborhood')
            """)
        )
        await db.execute(
            text("""
