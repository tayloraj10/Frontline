...
- **Native camera plugin not wired up for Android yet** (noted 2026-08-08). `@capacitor/camera` was added on `phase2/ios-setup` and the app code (`ContributionPanel.tsx`) already calls it on any native platform, but only the iOS side was synced/permissioned there (`Info.plist` keys added, `npx cap sync ios` run) — deliberately scoped to iOS-only per that branch's isolation rule. Still needed for Android: `npx cap sync android` to register the plugin, plus `CAMERA` and the Android 13+ granular media permissions (`READ_MEDIA_IMAGES`) in `AndroidManifest.xml`. Do this in a pass where an Android device is available to actually verify the permission prompts and camera/gallery flow.
