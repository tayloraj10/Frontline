...
- [x] Admin `game_settings` overhaul (migrations `056`/`057`, `engine-admin-changes` branch): settings tab reordered/grouped by category with units (min/hr/×/pts/m+ft) and live feet↔meters sync, `civic_action_value` row removed (both backend call sites already had a hardcoded fallback so removal is a no-op for non-cleanup contribution types), `hotspot_multiplier`→2 and `trash_report_value`→0.5 tuning applied. Fixed a bind-param type-truncation bug in the points-recompute SQL (`COUNT(*) * :trash_report_value` was silently floored to 0 for fractional values since Postgres inferred the bind type from `COUNT(*)`'s bigint — fixed via explicit `CAST(:trash_report_value AS numeric)`) in both the bulk recompute and the per-campaign spendable-points-toggle preview. The toggle-preview modal now matches the bulk-recompute modal's layout (separate Points/Spendable columns each with their own change column) and only lists users whose balance would actually change. Full test pass against `testing-strategy-game-settings-overhaul.md` completed 2026-08-03 (see `dev-docs/testing-strategy-game-settings-overhaul.md`); explicitly out of scope: recalibrating already-submitted bag/pound cleanup contributions when `small_bag_value`/`large_bag_value`/`pound_value`/`hotspot_multiplier`/`claim_challenge_multiplier` change, since those values are baked into `contributions.value` at submission time and there's no safe way to rewrite that history — decided against 2026-08-03, mitigated with an admin UI notice instead.
...
- [x] **Business "radius of influence" view + redemption-history detail** (built 2026-08-13, no migration — reuses `contributions.location GEOGRAPHY(POINT,4326)` and its existing GIST index) — two additions to `/partners/dashboard`. New `_is_business_admin()` gate helper in `backend/app/api/routes/partners.py` (this file previously had no permission checks on any route; added real ones here rather than replicating the gap for new code). **Radius view:** `GET /partners/businesses/{business_id}/locations/{location_id}/radius-stats?radius_tier=block|neighborhood|wide` — server-side tier→meters map (250/800/2400m, never client-supplied), `ST_DWithin` against nearby `contributions` per campaign the business is linked to via `campaign_partner_businesses` (confirmed via `EXPLAIN ANALYZE` to use `contributions_location_idx`, not a sequential scan). Sparse-data risk (a near-empty radius reading as "nothing is happening") solved by returning a citywide/campaign-wide comparison number alongside the local one, plus a `floor_state: "building"` for genuinely new locations/campaigns rather than a bare zero. New `BusinessRadiusView.tsx` (tier toggle, location picker if multiple locations). **Redemption history:** `GET /partners/businesses/{business_id}/redemptions?offer_id=&limit=&offset=` — joins `partner_offers`/`profiles`/`partner_business_locations`, deliberately a JOIN not LEFT JOIN on `partner_offers` so an offer going `expired` doesn't hide its title from historical redemption records (verified: an expired offer's title still returns correctly). New `RedemptionHistoryTable.tsx` (per-offer filter, paginated "Load more", used/unused status badge). Both are new tabs in `PartnerDashboardClient.tsx` alongside the existing Offers tab. No ShareCard/map-snapshot visual and no CSV export in v1 (deferred as fast-follows, not built). → `dev-backlog-2026-07-24.md` (item 9), master backlog item 3
...
- [x] **Report-a-photo (App Store UGC requirement):** every user-submitted photo surface now has a working flag/report control, closing the gap flagged during app-store-listing review (`problem_reports` photos already had their own flag mechanism and were out of scope here). New generic `content_flags` table (migration `071`) + `POST /api/content-flags` (`backend/app/api/routes/content_flags.py`), mirroring the existing `problem_report_flags` dedup-by-unique-constraint + threshold-based auto-hide pattern (`game_settings.flag_auto_hide_threshold`, default 3). Four `content_type`s cover every remaining photo surface: `contribution_photo` (map marker photos on collage/hex_bloom campaigns → nulls `contributions.photo_url` on auto-hide), `cleanup_log_photo` (contribution-derived entries in `cleanups.image_urls`, which have no per-row id of their own — `cleanup_events.py`'s photo aggregation now zips each flattened URL against its parent `cleanups.id` via `array_agg` so it can still be uniquely targeted → `array_remove`s the URL on auto-hide), `cleanup_event_photo` (standalone `cleanup_event_photos` rows → row delete on auto-hide), and `avatar` (`profiles.avatar_url` → nulled on auto-hide). New reusable `ReportPhotoButton.tsx` client component wired into: `CampaignMap.tsx`'s photo-marker lightbox (threaded a new `userId` prop down through `CampaignPageClient.tsx` → `CampaignMapWrapper.tsx`), `CleanupEventDetail.tsx`'s event photo gallery (via a new optional `renderActions` render-prop on the shared `Lightbox.tsx`, unused by `ContributionPanel.tsx`'s separate problem-report photo lightbox), and `users/[username]/page.tsx`'s profile avatar (shown to other logged-in users viewing someone else's profile, not the profile owner). Scoped out of this pass: `ZipPanel`/`HexPanel`'s secondary aggregated photo-thumbnail strips (`reportPhotos`, `cleanupPhotos`, `HexPhoto[]`) remain preview-only, since their underlying data carries no per-photo id to flag against.
- [ ] **Bug — problem report submissions can be silently rolled back by the push-notification cascade:** `submit_problem_report` (`problem_reports.py`) inserts the report and calls `_check_report_triggers` in the same DB transaction, committing both together. When a report crosses the hotspot threshold, the `campaign_events` insert fires `on_campaign_event_insert` → inserts into `user_notifications` → fires `on_user_notification_insert_push` (migrations `062`/`063`) → `net.http_post(...)`. Hit in prod 2026-08-08 (TestFlight): `pg_net` wasn't enabled on the prod Supabase project, so `net.http_post` threw `InvalidSchemaNameError: schema "net" does not exist`, which propagated up and rolled back the whole transaction — the user's problem report itself was lost, not just the hotspot event. Enabling `pg_net` in the Supabase dashboard fixed this specific trigger, but the underlying coupling is still there: **any** future failure in the notification/push chain (bad Vault secret, Edge Function down, FCM outage) will still silently eat a problem report submission the same way. Needs a real fix — wrap `_check_report_triggers` (or the notification cascade generally) so a failure there logs but doesn't roll back the primary report insert.
...
- [x] **Global Geo Stats page (`/leaderboard`) — interactive drill-down map + stat-card detail views** — extends the existing `GeoStatsExplorer.tsx` breakdown (interval filter, borough/neighborhood/zip drill-down list, top users/groups) with a visual layer. New `GET /api/geo-units/{geo_unit_id}/bbox` (`geo_units.py`) returns an arbitrary geo unit's centroid + bbox (existing endpoints only covered zip/UK-postcode-district lookups by code), used to `fitBounds`/`flyTo` the map when the drill-down focus changes. New `GeoStatsMap.tsx` renders the current level's MVT tiles (`nyc_boroughs`/`nyc_neighborhoods`/campaign territory tiles, reusing the existing tile endpoints — no new tile infrastructure) as a MapLibre choropleth: since tile features carry no app-level stat data, colors are computed client-side via a quantile-rank `match` expression keyed on `geo_unit_id` against an emerald ramp; clicking a shape calls the same `drillInto()` used by the list rows, so map and list navigation stay in sync. The 4 summary stat cards (pts/contributions/contributors/groups active) are now clickable, opening `StatDetailModal.tsx` with a distinct hand-rolled SVG visualization each (no charting library in the project): pts → 30-day daily trend bar chart backed by a new `GET /api/campaigns/{campaign_id}/geo-stats/trend` endpoint (scope-filtered identically to the main geo-stats endpoint via a new shared `_scope_filter()` helper in `leaderboard.py`); contributions → ranked bar chart of children breakdown by geo unit (falls back to ranked-by-contributor when no drill level is active); contributors/groups active → ranked bar charts from the already-fetched `top_users`/`top_groups`. Drilling all the way down to a terminal (zip-level) unit previously blanked the map entirely, since `level` becomes `null` once there's nothing left below to drill into — fixed by deriving a synthetic single-item "self view" `ChildUnit[]` from the focused unit + its own aggregate whenever `levelsBelow()` is empty, so the map still renders (highlighting just that unit, `onDrill` a no-op) instead of disappearing. `/geo-stats` also now returns `small_bags`/`large_bags`/`pounds` alongside `total_value` for the aggregate, `top_users`, `top_groups`, and `children` (zip-level via exact join, neighborhood/borough-level via a dedup-before-join CTE to avoid double-counting team-split cleanup events matched multiple times by the spatial join), surfaced in the UI as a "bags collected" summary tile and a conditional bags column on each breakdown/group/individual row (hidden on mobile, shown only when nonzero). Verified via `tsc --noEmit` (clean), `eslint` (no new errors beyond pre-existing `set-state-in-effect`/ref-during-render warnings already present elsewhere in these files), and live curl checks against the running dev backend confirming correct bag/pound totals at both campaign-wide and borough-scoped levels; not yet exercised in a live browser session.
- [x] **Group data portal (Phase 1 + CSV export)** — deep-dive stats page at `/groups/[slug]/stats`, reachable from a new "Stats" button on the group profile page's action row. New `GET /api/groups/{group_id}/stats` (`groups.py`, `_compute_group_stats`) reuses the `/geo-stats` dedup-before-join CTE pattern (avoids overcounting bag/pound metrics from team-split cleanup events) scoped by `group_id` instead of geography, with the same `interval` filter convention (today/week/month/all). Returns one block per distinct campaign the group has activity in rather than summing across campaigns — points/bags/lbs aren't comparable units across campaigns, same trap flagged for Community Goals scoping. All group members can view; `GET /api/groups/{group_id}/stats/export.csv` (gated via `_is_group_admin`) is admin-only. While scoping this, found and deliberately left unfixed a pre-existing gap: `/submit`'s self-logging path never server-side-checks that the caller is a member of the `group_id` they submit against — not fixed because the client UI (`ContributionPanel.tsx`) only ever offers the caller's actual `userGroups`, so it's not reachable through normal use; flagged for awareness if a public API surface is ever exposed. Not yet built: shareable visualization export, map snapshot export, Excel export, historical import — see `group-data-portal-scoping-2026-08-11.md`.
  - **Custom day/week/month navigation (Today/This Week/This Month/deep-dive pages):** both `/groups/[slug]/stats` and the admin deep-dive page previously only showed the *current* today/week/month period. `resolve_stats_window()` (new `backend/app/services/stats_window.py`) replaces the old `date_trunc(unit, now())`-with-no-upper-bound resolution used at every interval-filtered endpoint in `groups.py`, accepting an optional explicit `start_date`/`end_date` (YYYY-MM-DD) pair that takes priority over the bare `interval`, with a matching upper bound (`< end`) added everywhere a lower bound (`>= start`) already existed — letting any past day/week/month be queried, not just the current one. Trend-chart bucketing (`_compute_group_trend`, `get_group_geo_stats_trend`) switched from a literal `interval == "today" ? hour : day` check to a span-based rule (hour/day/week/month buckets keyed off the resolved window's actual length), since a custom range can now span anything from one day to years. Frontend: new shared `frontend/src/app/groups/[slug]/stats/statsWindow.ts` (`StatsWindow = { interval; anchor }`, `shiftAnchor`, `statsWindowParams`, `detailedIntervalLabel`) and `IntervalPicker.tsx` (existing Today/Week/Month/All pills plus, for non-"all" intervals, prev/next arrows and a native date-jump input) replace 4 independently-redeclared local `Interval` types across `GroupStatsView.tsx`, `GroupStatsAdminView.tsx`, `GroupGeoStatsExplorer.tsx`, and `GroupStatBreakdownCharts.tsx`; `MemberDrilldownModal.tsx` also wired through. "All Time" keeps no navigation, since it has no single anchor date.
  - **Admin deep-dive page: 7 usability fixes.** (1) Bags trend chart (`GroupTrendChart.tsx`) now renders small/large as two side-by-side bars per bucket (emerald/sky) with a small legend, instead of collapsing them into one bar. (2) Breakdown donut charts (`GroupStatBreakdownCharts.tsx`) gained "By Zip Code" (frontend-only, reuses the existing `children_level=zip` support on `/geo-stats`) and "By Type" slices — the latter backed by a new `GET /api/groups/{group_id}/stats/type-breakdown` endpoint grouping points by `CASE WHEN cleanup_event_id ... 'event' WHEN cleanup_id ... 'individual_log' ELSE 'manual'`. (3) The admin deep-dive's "Activity Map" section (`GroupEventsMap.tsx` — the events/contribution-points map, distinct from the "Geographic Deep Dive" section's `GeoStatsMap.tsx`) gained a compact legend strip under the map: green dot = contribution location, blue/amber/gray dots = upcoming/ongoing/past event. (The shared `GeoStatsMap.tsx`, used by both the "Geographic Deep Dive" section and `/leaderboard`, separately also gained its own legend strip — activity-ramp gradient plus, when rank coloring is on, gold/silver/bronze swatches — as part of item 4/5 work below, though the user's original "legend for the activity map" ask specifically meant the `GroupEventsMap.tsx` one.) (4) Fixed `GeoStatsMap.tsx`'s bbox-fit auto-zoom: `top3()` was including zero-value units in its top-3-by-value pick, so drilling into a sparsely-populated area could zoom to arbitrary empty units instead of falling back to the parent's full extent — now filters to `total_value > 0` first. (5) The group deep-dive's map previously passed `useRankColors={false}`, opting out of the gold/silver/bronze choropleth coloring that `/leaderboard`'s map already had by default (list rank badges were already consistent) — now matches. (6, 7) `GroupGeoStatsExplorer.tsx`'s and leaderboard `GeoStatsExplorer.tsx`'s top-members/individuals lists rendered plain initials instead of the shared `Avatar` component, even though `avatar_url` was already present in both `TopUser` payloads — both swapped to `<Avatar />`, matching the app's main members list and `MemberDrilldownModal.tsx`. ("Top Groups" on `/leaderboard` intentionally left as initials — represents groups, not users.)
- [x] **Event-redeemable offers** (migrations `081`-`083`, scoped in `event-redeemable-offers-and-nearby-business-suggestions-scoping-2026-08-26.md`) — lets a business offer attend-and-redeem freebies to a specific group cleanup event's attendees, resolving the doc's open question 1 toward a per-attachment model rather than either of the originally-proposed group-scoped or proximity-scoped designs: `partner_offers.event_eligible` (migration `081`) is an orthogonal flag (a spend/threshold offer can also be event-eligible), and an organizer attaches a specific event-eligible offer to their specific cleanup event via a new `cleanup_event_offers` junction row (created from `NearbyPartnersPanel.tsx`, shown in both `CreateCleanupEventForm.tsx` and `ContributionPanel.tsx`'s duplicated host-event flow) rather than the doc's proposed `event_group_id` (which would have made the offer apply to every event a group ever hosts). A pure `event_only` redemption mode (migration `082`, added to the existing `spend`/`threshold` CHECK) covers offers with no points price at all — `PartnersBrowseClient.tsx`, `CampaignMap.tsx`, and `campaigns/[slug]/page.tsx` all show an explanatory message instead of a disabled buy button for these. Eligibility at redemption time checks the attendee checked into the specific attached cleanup within `EVENT_OFFER_REDEMPTION_WINDOW = 4h` after the event's check-in window closes (`backend/app/api/routes/partners.py`), not the doc's proposed "any past event from the group within N days" — deliberately tighter, reading as "thank you for today's cleanup" rather than ongoing group VIP status. Redemption surfaces inside the event itself (`CleanupEventDetail.tsx`'s `EventOfferRedemptionCard`), resolving the doc's open question 5, not folded into the general points-redeemable browse list. Offers are still admin/business-dashboard-created (`OfferForm.tsx`), resolving open question 2 toward the doc's recommended v1 (no self-serve group-side offer creation). An organizer can optionally cap redemptions per event-attachment (`cleanup_event_offers.max_redemptions`, migration `083`, nullable = unlimited, default) — e.g. "only the first 20 attendees" — enforced in `redeem_offer` via a `SELECT ... FOR UPDATE`-then-count check (mirrors the existing `max_total_redemptions` pattern) against a new `partner_redemptions.cleanup_id` column (migration `083`) that attributes each event redemption to the specific event it came through; `CleanupEventDetail.tsx` shows "N of M left" / "Fully claimed" once hit. Piece 2 (nearby-business proximity suggestions at event creation, contact-email field, `GET /partners/nearby`) was not built in this pass — still open per the scoping doc. When an offer is attached to an event, the business's admins (To) and the organizer (CC) are emailed via a new `POST /partners/offers/{offer_id}/events/{cleanup_id}/notify-attachment`, called from both `CreateCleanupEventForm.tsx` and `ContributionPanel.tsx` right after their `cleanup_event_offers` insert; a failed send never blocks event creation. Email is sent through a new general-purpose `send_email()` service (`backend/app/services/email.py`) that calls Resend's HTTP API directly (no new SDK dependency), from `notifications@frontlinemaps.com`, BCC'd and Reply-To'd to `frontlinemapsapp@gmail.com` on every send regardless of kind, and logged to a new `emails_sent` table (migration `084`, no RLS, backend-only) so future email kinds reuse the same audit trail. Migrations `082`-`084` were applied to the local dev DB only (`supabase migration up --local`); not yet applied to prod.
