...
- [ ] **HIGH PRIORITY — Partner apply link missing from public Partners page** (noted 2026-08-09) — the new-partner-form link/button currently only lives on admin → partners; needs to be surfaced on the public-facing Partners page too so any user can self-serve apply/onboard as a partner, not just admins.
- [ ] **HIGH PRIORITY — Cleanup event check-in should award 5 points** (noted 2026-08-09) — when a user checks in to a cleanup event (self check-in or organizer checks them in), grant 5 points as an attendance incentive. Needs: (1) trigger on check-in, (2) UI feedback so both the user and organizer see the points were awarded, (3) the 5-point value added to the global points system, (4) surfaced/editable in admin → settings alongside the other point values.
