...
- [ ] **Restrict Firebase Android/iOS API keys** in Google Cloud Console (GitHub flagged both `google-services.json` and `GoogleService-Info.plist` keys as exposed secrets 2026-08-07 — not real secrets, but should be restricted to package/bundle ID + SHA-1 anyway). Android package `com.frontline.app`, debug SHA-1 `A7:7A:59:4B:F8:5F:DB:18:63:97:7A:AF:C1:A8:2E:23:A9:33:85:D0` — add the release keystore's SHA-1 too once it exists or signed release builds will fail the restriction. iOS: bundle ID `com.frontline.app`. → `android-release-checklist.md`, `ios-release-checklist.md`
