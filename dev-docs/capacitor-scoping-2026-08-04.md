...
3. [x] **Native project scaffolding**: `frontend/android/` and `frontend/ios/` generated and committed. iOS builds need the Mac laptop (confirmed available) — Xcode/CocoaPods/signing can't happen on Windows.
4. [x] **Icons/splash**: generated via `@capacitor/assets` from `frontend/src/app/icon-original.png` (copied to `frontend/resources/icon.png`). Full Android adaptive-icon/splash set, iOS AppIcon/Splash assets, and PWA icons (matched to the existing `public/manifest.webmanifest`, which was untracked until this commit) are all in place.
...
6. [ ] **Push notifications**: not started. Device token capture/storage on the backend, APNs cert (Apple) + FCM setup (Android/Google), a minimal trigger (even just "new points awarded" or similar) to justify inclusion for App Store review.
7. [ ] **Native share sheet**: not started. `@capacitor/share` on existing share buttons, if any exist, or add one to a campaign/event page.
