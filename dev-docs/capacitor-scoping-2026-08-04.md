...
6. [~] **Push notifications**: Firebase/FCM setup, device token storage, client-side plugin wiring, and the push-send Edge Function + DB trigger are all done. See `push-notifications-scoping-2026-08-06.md` for what's left: prod secrets/deploy (not yet done — needs explicit go-ahead), the Android manual test pass, and the iOS test pass (blocked on `ios/` scaffolding).
